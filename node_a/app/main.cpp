...
int main() {
    std::cout << "=== CAN-RTOS-Sim Node A (Sensor) ===" << std::endl;
    ...
    SensorTasks sensorTasks;
    if (!sensorTasks.initialize()) {
        std::cerr << "Failed to initialize sensor tasks" << std::endl;
        return 1;
    }
    
    std::cout << "Sensor tasks initialized" << std::endl;
    
    // Main loop
    std::cout << "Starting sensor node main loop..." << std::endl;
    ...
    while (running) {
        ...
        // Transmit sensor frames
        sensorTasks.transmitFrames();
        ...
        if (frameCount % 100 == 0) {
            ...
            std::cout << "Node A: " << frameCount << " frames sent, " 
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(10) - loopDuration);
    }
    
    std::cout << "Shutting down Node A..." << std::endl;
